...
import { SafeAreaView, FlatList, ActivityIndicator, Text, View, TouchableOpacity, Image } from "react-native";
import { useDispatch, useSelector } from 'react-redux';
import { postListLineRun } from '../../../redux/action';
import IntegrationListLineStyles from "../../../styles/IntegrationListLineStyles"; 
import AntDesign from 'react-native-vector-icons/AntDesign';
import MaterialCommunityIcons from 'react-native-vector-icons/MaterialCommunityIcons';
import FontAwesome5 from 'react-native-vector-icons/FontAwesome5';
import IntegrationStartLine from "../../Integration/IntegrationStartLine";
...
const Completed = ({ navigation }) => {
    ...
    useEffect(() => {
        if (postListLineRunData) {
            const filteredData = postListLineRunData.filter(item => item.Status === 'Completed');
            setCompletedData(filteredData);
        }
    }, [postListLineRunData,dispatch]);
        ...
        <View style={IntegrationListLineStyles.itemContainer}>
            ...
            <TouchableOpacity onPress={() => handleViewPress(item)}>
                ...
                <View style={IntegrationListLineStyles.item1Container}>
                    <View style={IntegrationListLineStyles.timeContainer}>
                        <Text style={IntegrationListLineStyles.itemText}>
                            <Text style={IntegrationListLineStyles.labelText}>LR No: </Text>
                            {item.LrNo}
                        </Text>
                        <Text style={IntegrationListLineStyles.itemText}>
                    ...
                    <View>
                        {item.Status === 'Completed' || item.Status === 'Running' ? (
                            ...
                            <View style={IntegrationListLineStyles.iconContainer}>
                                ...
                                <View style={IntegrationListLineStyles.iconWithText}>
                                    <TouchableOpacity style={IntegrationListLineStyles.iconButton} onPress={() => handleViewPress(item)}>
                                        <AntDesign name="eye" size={15} color="#f6a62e" />
                                        <Text style={IntegrationListLineStyles.startText}>View</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                        ) : null}
